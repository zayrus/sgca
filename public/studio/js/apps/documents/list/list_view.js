StudioManager.module("DocsApp.List", function(List, StudioManager, Backbone, Marionette, $, _){
  List.Layout = Marionette.Layout.extend({
    ...
    getTemplate: function(){
      return utils.templates.ProductionListLayoutView;
    },
...
*/
  ...
  List.Production = Marionette.ItemView.extend({
    tagName: "tr",

    getTemplate: function(){
      return _.template(utils.buildRowRenderTemplate(utils.productionsListTableHeader,utils.buildTableRowTemplates));
    },
/*
    template: _.template(
      '<td><%= cnumber %></td>' +
...
*/
    events: {
      ...
      "click button.js-show": "showClicked",
      "click button.js-edit": "editClicked",
      "click button.tlink": "editClicked",
      "click .js-zoom" : 'viewRelated',
      "change .tselect"  : "checkbox",
    },
    ...
    showClicked: function(e){
      console.log('showCLICKED in ITEM VIESW [%s]');
      e.preventDefault();
      e.stopPropagation();
      this.trigger("production:show", this.model);
    },
    ...
    editClicked: function(e){
      ...
      e.preventDefault();
      e.stopPropagation();
      this.trigger("production:edit", this.model);
    },
  ...
  var NoProductionsView = Marionette.ItemView.extend({
    template: _.template('<td colspan="3">No hay comprobantes para mostrar</td>'),
    tagName: "tr",
    className: "alert"
  });
  ...
  List.Productions = Marionette.CompositeView.extend({
    tagName: "table",
    className: "table table-bordered table-hover table-condensed",

    getTemplate: function(){
      return _.template(utils.buildTableHeader(utils.productionsListTableHeader)+'<tbody></tbody>');
    },
    ...
    emptyView: NoProductionsView,
    itemView: List.Production,
  ...
  List.queryForm = function(query, cb){
        var facet = new StudioManager.Entities.DocumQueryFacet(query ),
  ...
  List.groupEditForm = function(cb){
        var facet = new StudioManager.Entities.DocumGropuEditFacet(),
            form = new Backbone.Form({
                model: facet
            });
