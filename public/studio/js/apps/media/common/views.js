StudioManager.module("MediaApp.Common.Views", function(Views, StudioManager, Backbone, Marionette, $, _){
  ...
  Views.SearchItem = Marionette.ItemView.extend({
    templates: {
      ...
      productions:  _.template('<strong><%= ea_productionId %></strong>: <%= properties.name %>'),
    },
  ...
  Views.SearchPanel = Marionette.CompositeView.extend({
    ...
    documentList: function(){
      var criteria = $('input',this.$el).val(),
          self = this;
      console.log('media:views:filtered list SEARCH [%s]',criteria)

      StudioManager.request(this.options.searchtrigger, criteria, function(documents){
          console.log('media:views:Filtered CALLBACK: [%s]',documents.length);
...
*/ 
  ...
  Views.NavPanel = Marionette.CompositeView.extend({
    ...
    events: {
      "click .js-session-begin": "newsession",
      "click .js-session-push": "sessionpush",
      "click .js-new-user": "newuser",
      "click .js-new-ingest": "newingest",
      
      "click .js-prev": "fetchPrev",
      "click .js-next": "fetchNext",

      "click a.brand": "brandClicked",
      "click .js-filter-by-id" : "productionSearch",
      "click .js-groupedit": 'groupEdit',
    },

    newingest: function(e){
      e.preventDefault();
      this.trigger('production:new:ingest');
      return false;
    },

    newuser: function(e){
      e.preventDefault();
      this.trigger('production:new:user');
      return false;
    },

    sessionpush: function(e){
      e.preventDefault();
      console.log('SessionPush!!!');

      this.trigger('production:session:push');
      return false;
    },
    ...
    newsession: function(e){
      ...
      this.trigger('production:session:begin');
      return false;
    },
    ...
    productionSearch: function(){
      console.log('productionSearch: [%s]',$('input',this.$el).val())
      var self = this,
          query = this.$('#search-by-id').val();
      this.trigger('production:search', query, function(entity){
        console.log('views/productionSearch cb');
        //this.trigger('document:edit',entity);
      });
